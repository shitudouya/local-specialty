<template>
  <div>
    ...
    <div v-if="allList.length > 0">
      <div class="order-detail" v-for="item in allList" :key="item.order_id">
        <div class="top">
        ...
        <div class="bottom clearfix">
          ...
          <div class="right" v-if="item.status === 0">
            <div class="order-operate del" @click="handlePayOrder(item.order_id)">
              继续支付
            </div>
            <a-popconfirm
              title="确定要取消订单吗"
              ok-text="确定"
              cancel-text="暂时不"
              @confirm="handleCancelOrder(item.order_id)"
            >
              <div class="order-operate del del-two" style="margin:20px 0">
                取消订单
              </div>
            </a-popconfirm>
          </div>
          ...
          <div class="right" v-else-if="item.status === 3">
            <div class="order-operate del now" style="margin-top:16px" v-if="item._comments.length===0" @click="handleComment(item)">
              立即评价
            </div>
            <a-popconfirm
              title="确定要删除此订单吗"
...
export default {
  ...
  data() {
    return {
      ...
      previewImage: "",
      commentForm: this.$form.createForm(this, { name: "coordinated" }),
    };
  },
  computed: {
    timeFormat() {
