<template>
  <div class="navbar">
    ...
    <div class="right-menu">
      <template v-if="current_info.permission === 1">
        <el-badge :value="count" class="item" >
          <i class="el-icon-bell" @click="$router.push('/notify')"></i>
        </el-badge>
      </template>
      ...
      <template v-if="device !== 'mobile'">
        <div class="logout" title="退出登录" @click="handleLogout">
          <svg
            t="1616468133773"
            class="icon"
            fill="#525252"
...
export default {
  ...
  computed: {
    ...mapGetters(["sidebar", "device", "current_info"]),
  },
  mounted() {
    if (this.current_info.permission === 1) {
      this.getNotify();
    }
    ee.on("getNotify", () => {
      this.getNotify();
  ...
  methods: {
    ...
    handleCommand(command) {
      if (command === "organizationCheck") {
        this.$router.push("/organization/check");
      } else if (command === "schoolCheck") {
        this.$router.push("/school/check");
      }
    },
    handleLogout() {
